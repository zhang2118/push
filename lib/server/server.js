const instantEnvPrefix = 'unk';
try {
  if (process.env.FEATURE_FLAG_NAMESPACE) {
    instantEnvPrefix = process.env.FEATURE_FLAG_NAMESPACE;
  } else {
    instantEnvPrefix = "unk";
  }
}
catch (err) {
  instantEnvPrefix = "unk";
...
Push.appCollection = new Mongo.Collection(instantEnvPrefix+'_instant_push_app_tokens');
